    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {canManageProduction && !selectedPlan && <Button size="sm" onClick={() => setActiveView("setup")}><Plus className="mr-2 h-4 w-4" />Configuración</Button>}
        </div>
      </div>
      ...
      {activeView === "dashboard" && agencies.length === 0 && canManageProduction && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>Dos pasos y tendrás tu primer plan funcionando.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            <div className="flex items-start gap-3 rounded-lg border p-3">
              ...
              <div className="flex-1">
                <div className="text-sm font-medium">Crea la agencia</div>
                <p className="text-xs text-muted-foreground">El proveedor externo cuyo trabajo vas a controlar.</p>
              </div>
              <Button size="sm" onClick={() => setActiveView("setup")}>Crear agencia</Button>
            </div>
            <div className="flex items-start gap-3 rounded-lg border p-3 opacity-60">
              <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full border text-xs font-medium">2</span>
              <div className="flex-1">
                <div className="text-sm font-medium">Lanza un plan</div>
                <p className="text-xs text-muted-foreground">
                  Eliges agencia y plantilla, y se crean las etapas y los entregables solos.
                </p>
              </div>
              <Button size="sm" variant="outline" disabled>Lanzamiento rápido</Button>
            </div>
          </CardContent>
        </Card>
      )}
      ...
      {activeView === "dashboard" && agencies.length > 0 && (
        <DashboardView
...
function QuickLaunchDialog({ open, onClose, agencies, templates, onLaunch, launching }: {
  ...
  return (
    <Dialog open={open} onOpenChange={(isOpen) => !isOpen && onClose()}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Lanzamiento rápido</DialogTitle>
